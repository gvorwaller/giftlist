...
describe('matchSiblingsToShipment — td-d08902', () => {
	it('returns all siblings when the email enumerated no item titles', () => {
		// Legacy / single-item shipping notification with no item enumeration.
	...
	it('returns empty matched array when items present but none align (caller can fall back)', () => {
		// Email lists items but Phase A rejects all matches.
		const siblings = [gift(1, 'Endoscope Camera Kit')];
		const shippedTitles = ['Generic Gift Card']; // no anchor token overlap
		const result = matchSiblingsToShipment(siblings, shippedTitles);
		expect(result.itemsHadTitles).toBe(true);
