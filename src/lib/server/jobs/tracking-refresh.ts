...
import { isTrackingProviderConfigured, pullAllInFlight } from '../tracking';
...
/**
 * Polls the configured tracking provider (currently Shippo) for every gift
 * with a registered tracking id that hasn't reached a terminal status.
 * Idempotent — duplicate checkpoint events are dropped at the DB layer via
 * the UNIQUE constraint on shipment_events.
 *
 * No-ops with skipped=true if SHIPPO_API_KEY isn't configured, so this is
